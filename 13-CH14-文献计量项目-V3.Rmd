...
setwd("~/Nutstore Files/115_JournalPaper/11_biblio-aphasia")

tbl.fig = "data/TablesFigures/"
...
filenames = list.files(path = "data/scopus", 
                       pattern = "csv", 
                       full.names = TRUE)
# converting to bibliometric data frame
...
stopWords <- read_csv("data/stopWords.csv", col_names = FALSE)
...
citation.df = neuro.bib%>%
  ...
  mutate(vertex = str_remove(vertex, ".,"))


# a = aphasia.df %>%
#   mutate(vertex = str_replace_all(vertex, "\.",","))%>%
#   head(10)

CoCitation.raw = as.data.frame(CoCitation$cluster_res)%>%
  #mutate(vertex = str_replace_all(vertex, "[a-z]\\. ",", "))%>%
  mutate(
    #vertex = str_remove(vertex, "-[0-9]{1}$"),
         vertex1 = str_extract(vertex,"[a-z]{1,10} "),
...
aphasia.df = aphasia%>%
  tibble::rownames_to_column(., "vertex")%>%
  mutate(vertex = toupper(vertex))
