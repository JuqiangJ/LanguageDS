...
&emsp;&emsp;文本数据是语言数据的一个重要形式。从语体上来说，包括书面语文本和口语转写文本。从文本的产生来看，人们首先选择词汇，构成短语，组建句子，形成段落和语篇。从数据分析的角度，我们要将文本进行分词，或者对句子进行词性分析，然后从词句的特征进而得到语篇的特征。文本数据分析涉及到计算机学科的自然语言处理技术、语料库语言学的一些概念和理论，是一个综合的过程。同时不同语言中对于词句等层级的划分也各有不同，存在不同的技术难点。本章主要以英语和中文为例，探讨从词汇到短语，再到句子语篇的各个层级上，数据处理的方法。
